...
export function createTwitchVideosClient(options) {
    return {
        async getMetrics(request) {
            validateVideoMetricsRequest(request);
            const { accessToken, accessTokenProvider } = await resolveTwitchVideoAccessToken(options);
...
async function resolveTwitchVideoAccessToken(options) {
    ...
    throw new PlatformValidationError("Twitch appAccessToken or userAccessToken is required for videos.getMetrics().", {
        platform: TWITCH_PLATFORM,
    });
}
