...
export function validateYoutubeConfig(config) {
    ...
    const hasRefreshToken = "refreshToken" in config &&
        typeof config.refreshToken === "string" &&
        config.refreshToken.length > 0;
    const hasClientId = "clientId" in config &&
        typeof config.clientId === "string" &&
        config.clientId.length > 0;
    const hasClientSecret = "clientSecret" in config &&
    ...
    if (hasRefreshToken && (!hasClientId || !hasClientSecret)) {
        throw new PlatformValidationError("YouTube clientId and clientSecret are required when refreshToken is provided.", {
            platform: YOUTUBE_PLATFORM,
        });
    }
}
